...
# 第 0 步：定义任务路径
# -------------------------------
# Path resolution: env vars override hard-coded defaults so this script can
# be reused by the corrected reanalysis pipeline.
# 路径解析：环境变量优先于硬编码默认值，便于 corrected reanalysis 复用。
master_output_root <- "/Users/dingchenchen/Documents/New records/bird_new_records_R_output"
task_dir   <- Sys.getenv(
  "BIRD_TASK_DIR",
  unset = file.path(master_output_root, "tasks", "bird_sankey_order_province_year")
)
...
# --- topN + Others collapse: keep top N orders, group remainder as Others ---
# 把记录数最少的若干目合并为 Others，避免桑基图底部过度拥挤。
n_collapse <- as.integer(Sys.getenv("BIRD_SANKEY_N_COLLAPSE", unset = "5"))
order_rank_full <- clean %>% count(order, name = "n_records") %>% arrange(desc(n_records))
n_orders_total <- nrow(order_rank_full)
keep_orders <- order_rank_full$order[seq_len(max(1, n_orders_total - n_collapse))]
collapsed_orders <- setdiff(order_rank_full$order, keep_orders)
message("[sankey topN] keeping ", length(keep_orders), " orders; collapsing ",
        length(collapsed_orders), " into Others: ",
        paste(collapsed_orders, collapse = ", "))
clean <- clean %>%
  mutate(order = if_else(order %in% keep_orders, order, "Others"))

order_rank <- clean %>%
  count(order, name = "n_records") %>%
  arrange(desc(n_records)) %>%
  mutate(order_rank_idx = if_else(order == "Others", 999L, dplyr::row_number())) %>%
  arrange(order_rank_idx) %>% select(-order_rank_idx)
