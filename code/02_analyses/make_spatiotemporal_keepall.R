...
# 第 0 步：定义任务路径
# -------------------------------
# 路径解析：环境变量优先，便于多次重跑（keep-all-years 等）。
# Path resolution: env vars take priority, allowing multiple re-runs.
master_output_root <- "/Users/dingchenchen/Documents/New records/bird_new_records_R_output"
task_dir   <- Sys.getenv("BIRD_TASK_DIR",
  unset = file.path(master_output_root, "tasks", "bird_spatiotemporal_patterns"))
clean_path <- Sys.getenv("BIRD_CLEAN_PATH",
  unset = file.path(master_output_root, "data_clean", "bird_new_records_clean.csv"))
shape_dir  <- Sys.getenv("BIRD_SHAPE_DIR",
  unset = file.path(task_dir, "data", "shapefile_base"))
...
for (dir_path in c(figures_dir, code_dir, data_dir, results_dir)) {
